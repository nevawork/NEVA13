...
from core import VORTEXINF ,Cog ,Context 
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                error_details = (
                    ...
                    f"➡️ **Error Message:** {error_message}\n"
                    f"➡️ **Guild:** {ctx.guild.name} ({ctx.guild.id})\n"
                    f"➡️ **Channel:** {ctx.channel.name} ({ctx.channel.id})\n"
                    f"➡️ **User:** [{ctx.author}](https://discord.com/users/{ctx.author.id}) ({ctx.author.id})"
                )
                container.add_item(
                    discord.ui.Section(
