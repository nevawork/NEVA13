...
async def do_removal (ctx ,limit ,predicate ,*,before =None ,after =None ):
  ...
  temp_container.add_item(
      ui.TextDisplay(f"✅ | {deleted_count} message{' was' if deleted_count == 1 else 's were'} removed.")
  )
...
class PurgeAmountModal(ui.Modal, title="Purge Amount"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if amount < 1 or amount > 2000:
                await interaction.response.send_message(
                    "•  Amount must be between 1 and 2000.", 
                    ephemeral=True
                )
                return
        except ValueError:
        ...
        if self.purge_type == "user":
            ...
            if not user_input:
                await interaction.response.send_message(
                    "•  Please enter a user ID or mention.", 
                    ephemeral=True
                )
                return
            ...
            if not user:
                await interaction.response.send_message(
                    "•  User not found. Please enter a valid user ID or mention.", 
                    ephemeral=True
                )
                return
        ...
        if self.purge_type == "all":
            ...
            await self.view.ctx.send(f"✅ | Successfully removed {total_reactions} reactions.", delete_after=7)
...
class PurgeView(ui.LayoutView):
    ...
    def setup_main_content(self):
        ...
        if self.bot.user.avatar:
            self.container.add_item(
                ui.Section(
                    ui.TextDisplay("•  **Advanced message management system**\n•  **Supports up to 2000 messages per operation**\n•  **Multiple filtering options available**\n•  **Safe and logged operations**"),
    ...
    def setup_completion_content(self):
        ...
        operation_details = f"•  **Operation Type:** {self.purged_operation}\n•  **Messages Processed:** {self.purged_amount}\n•  **Channel:** {self.ctx.channel.mention}\n•  **Executed:** <t:{int(time.time())}:R>"
  ...
  async def clear (self ,ctx ,choice :Union [discord .Member ,int ]=None ,amount :int =None ):
        if choice is None:
            loading_container = ui.Container(
                ui.TextDisplay("⏳ Loading purge interface..."),
                accent_color=None
            )
