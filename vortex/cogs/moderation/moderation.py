...
class Moderation (commands .Cog ):
  ...
  async def unlockall (self ,ctx ):
      if ctx .author ==ctx .guild .owner or ctx .author .top_role .position >ctx .guild .me .top_role .position :
          ...
          async def button_callback (interaction :discord .Interaction ):
              ...
              if interaction .user ==ctx .author :
                  if interaction .guild .me .guild_permissions .manage_roles :
                    ...
                    await interaction .response .edit_message (
                    content =
                    "⚠️ | It seems I'm missing the necessary permissions. Please grant me the `manage roles` permissions and try again.",
                    embed =None ,
                    view =None )
              else :
          ...
          embed5 =discord .Embed (title ="⚠️ Access Denied",
  ...
  async def lockall (self ,ctx ):
      if ctx .author ==ctx .guild .owner or ctx .author .top_role .position >ctx .guild .me .top_role .position :
          ...
          denied =discord .Embed (title ="⚠️ Access Denied",
          description ="Your role should be above my top role.",
          color =0x000000 )
  ...
  async def give (self ,ctx ,member :discord .Member ,*,role :discord .Role ):
    ...
    try :
        ...
        error =discord .Embed (
        color =self .color ,
        description ="⚠️ I don't have permission to manage roles for this user!"
        )
        pass 
    except Exception as e :
        error =discord .Embed (
          ...
          async def button_callback (interaction :discord .Interaction ):
              ...
              if interaction .user ==ctx .author :
                  if interaction .guild .me .guild_permissions .manage_roles :
                    ...
                    await interaction .response .edit_message (
                    content =
                    "⚠️ | It seems I'm missing the necessary permissions. Please grant me the `manage channels` permissions and try again.",
                    embed =None ,
                    view =None )
              else :
                await interaction .response .send_message ("Oops! It looks like that message isn't from you. You need to run the command yourself to interact with it.",
  ...
  async def changenickname (self ,ctx :commands .Context ,member :discord .Member ,*,name :str =None ):
    ...
    try :
        ...
        error =discord .Embed (
        color =self .color ,
        description ="⚠️ | I don't have permission to manage this user's nickname!"
        )
        pass 
    except Exception as e :
        error =discord .Embed (
        ...
        async def yes_delete_callback (confirm_interaction :discord .Interaction ):
          if confirm_interaction .user ==ctx .author :
            if confirm_interaction .guild .me .guild_permissions .manage_channels :
              ...
              content ="⚠️ | I don't have permission to manage channels.",
              embed =None ,view =None )
          else :
            await confirm_interaction .response .send_message("Only the command author can use this button.",ephemeral =True )
  ...
  async def auditlog (self ,ctx ,limit :int ):
    if limit >=31 :
      await ctx .reply("Action rejected, you are not allowed to fetch more than `30` entries.",
