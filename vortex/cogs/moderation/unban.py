...
class BanContainer(ui.Container):
    def __init__(self, user, author):
        ...
        self.ban_row.add_item(ui.Button(emoji="🗑️", style=discord.ButtonStyle.gray, custom_id="delete_message"))
...
class UnbanSuccessContainer(ui.Container):
    def __init__(self, user, author, dm_status, reason):
        ...
        self.action_row.add_item(ui.Button(emoji="🗑️", style=discord.ButtonStyle.gray, custom_id="delete_message"))
...
class ReasonModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            await self.user.send(f"⚠️ You have been Banned from **{self.author.guild.name}** by **{self.author}**. Reason: {reason}")
            dm_status = "Yes"
