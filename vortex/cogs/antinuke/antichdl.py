        ...
        try :
            ...
            for cat_data in snapshot ['categories']:
                try :
                    ...
                    if recovery_channel :
                        await recovery_channel .send (f"✅ Restored category: **{cat_data['name']}**")

                except Exception as e :
                    if recovery_channel :
                        await recovery_channel .send (f"❌ Failed to restore category {cat_data['name']}: {e}")
            ...
            for ch_data in snapshot ['text_channels']:
                try :
                    ...
                    if recovery_channel :
                        await recovery_channel .send (f"❌ Failed to restore text channel {ch_data['name']}: {e}")
            ...
            for ch_data in snapshot ['voice_channels']:
                try :
                    ...
                    if recovery_channel :
                        await recovery_channel .send (f"❌ Failed to restore voice channel {ch_data['name']}: {e}")

            if recovery_channel :
                await recovery_channel .send ("✅ **FULL SERVER RECOVERY COMPLETED**\n"
                "🔒 Server has been secured against further attacks.\n"
                "⚠️ All suspicious bots have been banned.")
