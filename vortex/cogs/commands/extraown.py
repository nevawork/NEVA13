...
class Extraowner (commands .Cog ):
    ...
    async def extraowner (self ,ctx ,option :str =None ,user :discord .Member =None ):
        ...
        if ctx .guild .member_count <2 :
            ...
            container.add_item(ui.TextDisplay("❌ | Your Server Doesn't Meet My 30 Member Criteria"))
        ...
        if option is None :
            ...
            main_info = (
                ...
                f"➡️ `{pre}extraowner set @user` - Set Extra Owner\n"
                f"➡️ `{pre}extraowner reset` - Remove Extra Owner\n"
                f"➡️ `{pre}extraowner view` - View Current Extra Owner"
            )
        ...
        if option .lower ()=='set':
            ...
            if view.value is None:
                ...
                container = ui.Container(accent_color=None)
                container.add_item(ui.TextDisplay("# ❌ Cancelled"))
                container.add_item(ui.Separator())
