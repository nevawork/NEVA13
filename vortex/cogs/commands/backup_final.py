...
class Backup (commands .Cog ):
    ...
    async def safe_delete (self ,obj ,status_channel ):
        ...
        try :
            ...
            await self .post_status (status_channel ,f"⚠️ Failed to delete {obj}: {e}",0x000000 )
        ...
        for attempt in range (retries ):
            try :
                ...
                if e .status ==429 and attempt <retries -1 :
                    ...
                    await self .post_status (status ,f"⚠️ Rate limit on emoji {name}, retrying in {retry_after}s...",0x000000 )
                    await asyncio .sleep (retry_after )
                    continue 
                await self .post_status (status ,f"⚠️ Failed to create emoji {name}: {e}",0x000000 )
                return False 
    ...
    async def delete (self ,ctx ,backup_id :str ):
        try :
            ...
            if not (self.is_admin(ctx) or backup["creator_id"] == ctx.author.id):
                return await ctx .send ("❌ Must be creator, admin, or owner to delete.")
    ...
    async def transfer (self ,ctx ,backup_id :str ,user :discord .User ):
        ...
        try :
            ...
            if backup["creator_id"] != ctx.author.id and not self.is_admin(ctx):
                 return await ctx .send ("❌ Not owned by you.")
    ...
    async def export (self ,ctx ,backup_id :str ):
        ...
        try :
            ...
            if len (backup_data )>8 *1024 *1024 :
                return await ctx .send ("❌ Backup too large to export.")
        ...
        try :
            ...
            for field in required_fields :
                if field not in backup_data or not isinstance (backup_data [field ],(list ,int ,str )):
                    return await ctx .send (f"❌ Invalid backup: '{field}' missing or incorrect.")
            ...
            async with aiosqlite.connect("db/backups.db") as db:
                ...
                async with db.execute("SELECT COUNT(*) as count FROM server_backups WHERE creator_id = ?", (ctx.author.id,)) as cursor:
                    row = await cursor.fetchone()
                    if row and row["count"] >= 2:
                        return await ctx .send ("❌ You have 2 backups. Delete one first.")
    ...
    async def backupdatabase (self ,ctx ):
        ...
        if ctx .guild .id !=GUILD_ID :
            return await ctx .send (f"❌ Only usable in guild ID: {GUILD_ID}.")
        if not await self .check_db_connection ():
            return await ctx .send ("❌ Database unavailable.")
        try :
            ...
            async with aiosqlite.connect("db/backups.db") as db:
                ...
                await db.commit()
                deleted_count = db.total_changes
            await ctx .send (f"✅ {deleted_count} backups deleted.")
        except Exception as e:
    ...
    async def load (self ,ctx ,backup_id :str ):
        if not self .is_admin (ctx ):
            return await ctx .send ("❌ Administrator permissions needed.")
        try :
            async with aiosqlite.connect("db/backups.db") as db:
                db.row_factory = aiosqlite.Row
                async with db.execute("SELECT * FROM server_backups WHERE backup_id = ?", (backup_id,)) as cursor:
        ...
        class ConfirmView (ui .LayoutView ):
            ...
            async def select_callback (self ,interaction :discord .Interaction ):
                if interaction .user !=ctx .author :
                    await interaction .response .send_message ("❌ Only the command author can choose.",ephemeral =True )
                    return 
                await interaction .response .defer (ephemeral =True )
                self .selection =interaction .data ["values"]
                await interaction .followup .send (f"✅ Selected: {', '.join(self.selection)}. Click Confirm.",ephemeral =True )
            ...
            async def cancel_callback (self ,interaction :discord .Interaction ):
                if interaction .user !=ctx .author :
                    ...
                    return 
                await interaction .response .send_message ("❌ Backup loading cancelled.",ephemeral =True )
                self .selection =None 
                self .stop ()
        ...
        if view .selection is None :
            return await ctx .send ("❌ Backup loading timed out."if view .timed_out else "❌ Backup loading cancelled.")

        if not ctx .guild .me .guild_permissions .manage_channels :
            return await ctx .send ("❌ Bot lacks manage channels permission.")
        ...
        try :
            ...
            await self .post_status (status ,f"⚠️ Error deleting parts: {e}",0x000000 )
            return 
        ...
        if not bot_member :
            await self .post_status (status ,"⚠️ Bot member not found.",0x000000 )
            return 
        ...
        for r in sorted (roles_data,key =lambda x :x ["position"],reverse =True ):
            if not isinstance (r ["position"],int )or r ["position"]<1 :
                await self .post_status (status ,f"⚠️ Invalid position for role {r['name']}. Setting to 1.",0x000000 )
                r ["position"]=1 
            try :
                ...
                await self .post_status (status ,f"⚠️ Failed to create role {r['name']}: {e}",0x000000 )
            ...
            for role ,pos ,orig_pos in sorted (role_positions ,key =lambda x :x [1 ],reverse =True ):
                try :
                    ...
                    if role .position !=pos :
                        ...
                        await self .post_status (status ,f"⚠️ Role {role.name} at {role.position}, expected {pos}",0x000000 )
                except discord .HTTPException as e :
                    logger .error (f"Failed to position role {role.name}: {e}")
                    await self .post_status (status ,f"⚠️ Failed to position role {role.name}: {e}",0x000000 )
        except Exception as e :
            logger .error (f"Role positioning error: {e}")
            await self .post_status (status ,f"⚠️ Failed to set role positions: {e}",0x000000 )
        ...
        for cat_data in sorted (categories_data,key =lambda x :x ["position"]):
            try :
                ...
                await self .post_status (status ,f"⚠️ Failed to create category {cat_data['name']}: {e}",0x000000 )
        ...
        for ch_data in channels_data:
            try :
                ...
                await self .post_status (status ,f"⚠️ Failed to create channel {ch_data['name']}: {e}",0x000000 )
            ...
            try :
                async with self.bot.session.get(e_data["url"]) as resp:
                    if resp .status ==200 :
                        ...
                        await self .post_status (status ,f"⚠️ Failed to fetch emoji {e_data['name']} image: HTTP {resp.status}",0x000000 )
            except aiohttp .ClientError as e :
                await self .post_status (status ,f"⚠️ Failed to fetch emoji {e_data['name']} image: {e}",0x000000 )

        await self .post_status (status ,"✅ Backup restoration complete!",0x000000 )
