...
class Nightmode (commands .Cog ):
    ...
    async def nightmode (self ,ctx ):
        ...
        main_info = (
            ...
            "**Available Commands:**\n"
            f"➡️ `{ctx.prefix}nightmode enable` - Enable nightmode\n"
            f"➡️ `{ctx.prefix}nightmode disable` - Disable nightmode"
        )
    ...
    async def enable_nightmode (self ,ctx ):
        ...
        ctx .guild .me .top_role .position <=ctx .author .top_role .position 
            ...
            view = ui.LayoutView()
            container = ui.Container(accent_color=None)
            container.add_item(ui.TextDisplay("# ⚠️ Access Denied"))
            container.add_item(ui.Separator())
        ...
        success_info = (
            ...
            "**Protected Roles:**\n" + 
            "\n".join([f"✅ {role.mention}" for role in manageable_roles[:5]]) +
    ...
    async def disable_nightmode (self ,ctx ):
        ...
        success_info = (
            ...
            "**Restored Roles:**\n" + 
            "\n".join([f"✅ {role.mention}" for role in restored_roles[:5]]) +
