...
class TimeSelect(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(description=f"**Added Global No Prefix**:\n➡️ **User**: **[{self.user}](https://discord.com/users/{self.user.id})**\n➡️ **User Mention**: {self.user.mention}\n➡️ **User ID**: {self.user.id}\n\n__**Additional Info**__:\n **Added By**: **[{self.author.display_name}](https://discord.com/users/{self.author.id})**\n➡️ **Expiry Time:** {expiry_text}\n➡️ **Timestamp:** {expiry_timestamp}", color=0x000000)
...
class NoPrefix(commands.Cog):
            ...
            if expired_users:
                ...
                for user_id in expired_users:
                    ...
                    if user:
                        ...
                        embed.set_footer(text="VORTEXINF-bot  - No Prefix, Join support to regain access.")
            ...
            async with db.execute("SELECT id FROM np") as cursor:
                ...
                if not ids:
                    ...
                    empty_container = ui.Container(
                        ui.TextDisplay("-# 💠 No Prefix Users"),
                ...
                list_container = ui.Container(
                    ui.TextDisplay(f"-# 💠 No Prefix Users [{len(ids)}]"),
    ...
    async def np_add(self, ctx, user: discord.User):
        ...
        async def select_callback(interaction: discord.Interaction):
            ...
            success_container = ui.Container(
                ui.TextDisplay("-# 💠 Added No Prefix"),
                ui.TextDisplay(f"**Added Global No Prefix**:\n➡️ **User**: **[{user}](https://discord.com/users/{user.id})**\n➡️ **User Mention**: {user.mention}\n➡️ **User ID**: {user.id}\n\n__**Additional Info**__:\n **Added By**: **[{ctx.author.display_name}](https://discord.com/users/{ctx.author.id})**\n➡️ **Expiry Time:** {expiry_text}\n➡️ **Timestamp:** {expiry_timestamp}"),
                accent_color=None
            )
    ...
    async def np_remove(self, ctx, user: discord.User):
        ...
        # Create success container
        success_container = ui.Container(
            ui.TextDisplay("-# 💠 Removed No Prefix"),
        ...
        async with aiosqlite.connect('db/np.db') as db:
            ...
            if not result:
                ...
                error_container = ui.Container(
                    ui.TextDisplay("-# 💠 No Prefix Status"),
            ...
            status_container = ui.Container(
                ...
                ui.TextDisplay(
                    f"**👤 User**: [{user}](https://discord.com/users/{user.id})\n"
                    f"**💬 User ID**: {user.id}\n\n"
                    f"**⏱️ Expiry**: {expire_time} ({expire_timestamp})"
                ),
                accent_color=None
            )
