...
class ServerinfoLayout(ui.LayoutView):
    ...
    def setup_statistics_content(self):
        ...
        if self.guild.features:
            features = "\n".join([f"✅ {feature[:1].upper() + feature[1:].lower().replace('_', ' ')}" for feature in self.guild.features[:10]])
...
class Extra (commands .Cog ):
  ...
  async def uptime (self ,ctx ):
      ...
      container.add_item(ui.TextDisplay("# ⏰ VORTEXINF-bot Uptime"))
      container.add_item(ui.Separator())

      uptime_info = f"⚠️ **UTC:** {uptime_string}\n\n📌 **Online Duration:** {uptime_duration_string}"
  ...
  async def boosts (self ,ctx ):
    ...
    container.add_item(ui.TextDisplay(f"# 📌 Boosts Count Of {ctx.guild.name}"))
    container.add_item(ui.Separator())
    ...
    async def overwrites_callback(interaction):
        ...
        for target, perms in channel.overwrites:
            ...
            overwrites.append(f"**For {target.name}**\n" +
                "\n".join(f"  * **{perm}:** {'✅' if value else '❌' if value is False else '⛔'}"
        ...
        overwrites_container = ui.Container(
            ...
            ui.Separator(),
            ui.TextDisplay("✅= Allowed, ❌ = Denied, ⛔ = None"),
            accent_color=None
        )
