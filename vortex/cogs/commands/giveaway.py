...
class Giveaway(Cog):
        ...
        try:
            ...
            if not entries_list:
                ...
                container.add_item(ui.TextDisplay(
                    ...
                    f"• **Ended:** <t:{int(datetime.datetime.now().timestamp())}:R>\n"
                    f"• **Hosted by:** <@{host_id}>\n\n"
                    f"**Giveaway Ended!**"
                ))
            ...
            channel_container.add_item(ui.TextDisplay(
                f"Congrats! ⏱️ {', '.join(winner_links)}, you've won the **{prize}** ❤️, hosted by {host_link}"
            ))
                ...
                try:
                    ...
                    if user:
                        ...
                        dm_container.add_item(ui.TextDisplay(
                            f"⏱️ You won **{prize}** in **{guild.name}** ❤️"
                        ))
    ...
    async def gstart(self, ctx: Context, time: str, winners: int, *, prize: str):
        ...
        container = ui.Container(accent_color=None)
        container.add_item(ui.TextDisplay(f"• **{prize}** •"))
    ...
    async def greroll(self, ctx: Context, message: discord.Message | None = None):
        ...
        async with aiosqlite.connect('db/giveaways.db') as db:
            ...
            container = ui.Container(accent_color=None)
            container.add_item(ui.TextDisplay(f"• **New Winners!** •"))
            container.add_item(ui.Separator())
            container.add_item(ui.TextDisplay(
                f"**Prize:** {giveaway[5]}\n"
            ...
            for winner in winners:
                try:
                    ...
                    if user and guild:
                        dm_container = ui.Container(accent_color=None)
                        dm_container.add_item(ui.TextDisplay(
                            f"⏱️ You won **{giveaway[5]}** in **{guild.name}** ❤️"
                        ))
