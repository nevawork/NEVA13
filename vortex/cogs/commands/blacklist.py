...
from utils .paginator import Paginator as aegis 
...
class Blacklist (commands .Cog ):
    ...
    async def remove (self ,ctx ,word :str ):
        ...
        if not await self .is_word_blacklisted (guild_id ,word .lower ()):
            embed =discord .Embed (description =f"`{word}` is not in the blacklist.",
            color =discord .Color .from_rgb (0 ,0 ,0 )
            )
            await ctx .reply (embed =embed )
            return 
    ...
    async def reset (self ,ctx ):
        ...
        if not words :
            embed =discord .Embed (description ="No words are currently blacklisted.",
            color =discord .Color .from_rgb (0 ,0 ,0 )
            )
            await ctx .reply (embed =embed )
            return 
    ...
    async def bypass_add (self ,ctx ,target :Union [discord .Member ,discord .Role ]):
        ...
        if isinstance (target ,discord .Member ):
            ...
            if await self .is_role_bypassed (guild_id ,target .id ):
                embed =discord .Embed (description =f"`{target}` is already bypassed.",
                color =discord .Color .from_rgb (0 ,0 ,0 )
                )
                await ctx .reply (embed =embed )
                return 
    ...
    async def bypass_remove (self ,ctx ,target :Union [discord .Member ,discord .Role ]):
        ...
        if isinstance (target ,discord .Member ):
            if not await self .is_user_bypassed (guild_id ,target .id ):
                embed =discord .Embed (description =f"`{target}` is not bypassed.",
                color =discord .Color .from_rgb (0 ,0 ,0 )
                )
                await ctx .reply (embed =embed )
                return 
    ...
    async def bypass_list (self ,ctx ):
        ...
        if not users and not roles :
            embed =discord .Embed (description ="No users or roles are currently bypassed.",
            color =discord .Color .from_rgb (0 ,0 ,0 )
            )
            await ctx .send (embed =embed )
            return 
