...
class AutoResponder (commands .Cog ):
    ...
    async def _create (self ,ctx ,name ,*,message ):
        ...
        async with aiosqlite .connect (DB_PATH )as db :
            ...
            await db .commit ()
            await self.send_message(ctx, title="✅| Success", description=f"Created autoresponder `{name}` in {ctx.guild.name}")
    ...
    async def _delete (self ,ctx ,name ):
        ...
        async with aiosqlite .connect (DB_PATH )as db :
            ...
            await db .commit ()
            await self.send_message(ctx, title="✅| Success", description=f"Deleted autoresponder `{name}` in {ctx.guild.name}")
    ...
    async def _edit (self ,ctx ,name ,*,message ):
        ...
        async with aiosqlite .connect (DB_PATH )as db :
            ...
            await db .commit ()
            await self.send_message(ctx, title="✅| Success", description=f"Edited autoresponder `{name}` in {ctx.guild.name}")
