...
class Invcrole (commands .Cog ):
    ...
    async def add (self ,ctx ,role :discord .Role ):
        if role >= ctx.author.top_role and ctx.author != ctx.guild.owner:
            embed = discord.Embed(title="⚠️ Access Denied", description=f"You cannot add a role that is higher or equal to your own top role.", color=0x000000)
            return await ctx.reply(embed=embed)
        if role >= ctx.guild.me.top_role:
            embed = discord.Embed(title="⚠️ Access Denied", description=f"I cannot assign {role.mention} because it is higher or equal to my own top role.", color=0x000000)
            return await ctx.reply(embed=embed)
        if role.is_bot_managed() or role.is_integration() or role.is_premium_subscriber():
            embed = discord.Embed(title="⚠️ Access Denied", description=f"You cannot use integrated/managed roles as VC roles.", color=0x000000)
            return await ctx.reply(embed=embed)
    ...
    async def remove (self ,ctx ,role :discord .Role ):
        async with aiosqlite .connect (self .db_path )as db :
            async with db .execute ('SELECT role_id FROM vcroles WHERE guild_id = ? AND role_id = ?',(ctx .guild .id ,role .id ))as cursor :
                row =await cursor .fetchone ()
                if not row :
                    embed =discord .Embed (description ="Given role is not set in VC role.",color =0x000000 )
    ...
    async def config (self ,ctx ):
        async with aiosqlite .connect (self .db_path )as db :
            async with db .execute ('SELECT role_id FROM vcroles WHERE guild_id = ?',(ctx .guild .id ,))as cursor :
                row =await cursor .fetchone ()
                if not row :
                    embed =discord .Embed (description ="VC role is not set in this guild.",color =0x000000 )
                    await ctx .send (embed =embed )
                    return 
            ...
            async with aiosqlite .connect (self .db_path )as db :
                async with db .execute ('SELECT role_id FROM vcroles WHERE guild_id = ?',(member .guild .id ,))as cursor :
                    ...
                    if after .channel and role not in member .roles :
                        await self .add_role_with_retry (member ,role ,reason ="Member Joined VC | VORTEXINF Invcrole")
                    elif not after .channel and role in member .roles :
                        await self .remove_role_with_retry (member ,role ,reason ="Member Left VC | VORTEXINF Invcrole")
