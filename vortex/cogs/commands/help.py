...
from core .VORTEXINF import VORTEXINF 
...
class HelpLayout(ui.LayoutView):
    def __init__(self, help_cog, ctx, server_prefix=None):
        ...
        self.filtered_mapping = self.create_VORTEXINF_mapping()
    ...
    def create_VORTEXINF_mapping(self):
        """Create a filtered mapping that only includes cogs from cogs/VORTEXINF directories"""
        main_menu_classes = {
        ...
        for cog in self.ctx.bot.cogs.values():
            if cog and hasattr(cog, '__class__'):
                ...
                if cog_class_name in all_allowed_classes and hasattr(cog, 'help_custom'):
                    VORTEXINF_mapping[cog] = cog.get_commands()

        return VORTEXINF_mapping
    ...
    def setup_main_content(self):
        ...
        self.container.add_item(
            ui.TextDisplay(
                f"• **Type `{display_prefix}help` to get more info**\n"
                f"• **Total Commands available: `{self.total_commands}`**\n"
                f"• [Invite]({invite_link}) | [Website]({website_link}) | [Support]({support_link})"
            )
        )
        ...
        new_content = (
            f"⏱️ Hey [{self.ctx.author.display_name}]({self.ctx.author.avatar.url if self.ctx.author.avatar else 'https://discord.com'})!\n"
            f"I'm **VORTEXINF**, your friendly companion.\n\n"
            f"   ➡️ Prefix for this server: **{self.server_prefix}**\n"
            f"   ➡️ Pick from the menu to continue!"
        )
...
class HelpCommand (commands .HelpCommand ):
  ...
  def create_VORTEXINF_mapping (self ,mapping ):
    """Create a filtered mapping that only includes cogs from cogs/VORTEXINF directories"""
    VORTEXINF_mapping ={}
    ...
    for cog ,commands in mapping .items ():
      if cog and hasattr (cog ,'__class__'):
        ...
        if cog_class_name in allowed_cog_classes and hasattr (cog ,'help_custom'):
          VORTEXINF_mapping [cog ]=commands 
