...
class Automod (commands .Cog ):
        ...
        async def enable_logging_callback (interaction ):
            ...
            try :
                for channel in interaction .guild .channels :
                    if channel .name =="VORTEXINF-automod":
                        await interaction .response .send_message (f"A logging channel with the name \"VORTEXINF-automod\" already exists.",ephemeral =True )
                        return 
                log_channel =await interaction .guild .create_text_channel ("VORTEXINF-automod",overwrites =overwrites )
    ...
    async def ignore_show (self ,ctx ):
        ...
        if not ignored_items :
            await ctx .reply ("No ignored channels, roles, or users found for this server.")
            return 
    ...
    async def unignore_role (self ,ctx ,role :discord .Role ):
        ...
        if result .rowcount >0 :
            ...
            embed =discord .Embed (description =f"{role.mention} is not in the automod ignore list.",color =0x000000 )
            embed .set_footer (text =f"“{ctx.command.qualified_name}” Command executed by {ctx.author}",
            icon_url =ctx .author .avatar .url if ctx .author .avatar else ctx .author .default_avatar .url )
            await ctx .send (embed =embed )
