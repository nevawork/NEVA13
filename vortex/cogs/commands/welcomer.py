...
class VariableButton (Button ):
    ...
    async def callback (self ,interaction :discord .Interaction ):
        ...
        "{joindate}":"The user's join date in the server (formatted as Day, Month Day, Year).",
        "{user_createdate}":"The user's account creation date (formatted as Day, Month Day, Year).",
        "{server}":"The server's name.",
        "{serverid}":"The server's ID number.",
        "{count}":"The server's total member count.",
...
class Welcomer (commands .Cog ):
    ...
    def safe_format (self ,text ,placeholders ):
    ...
    @commands .group (invoke_without_command =True ,name ="welcome",aliases =["greet"])
    ...
    async def greet_setup (self ,ctx ):
        ...
        if row :
            ...
            embed =discord .Embed (
            title ="Welcome System Already Configured",
            description =f"A welcome message is already set up for {ctx.guild.name}.",
            color =0x000000 
            )
            ...
            async def reconfigure_callback (interaction :discord .Interaction ):
                ...
                await self ._start_setup_flow (ctx )

            async def view_config_callback (interaction :discord .Interaction ):
                if interaction .user !=ctx .author :
                    await interaction .response .send_message ("You cannot interact with this setup.",ephemeral =True )
                    return 
                ...
                if welcome_type =="simple":
                    ...
                    title ="Current Welcome Configuration (Simple)",
                    description =f"**Message:** {welcome_message[:1000] + '...' if len(welcome_message) > 1000 else welcome_message}",
                    color =0x000000 
                    )
                else :
                    if embed_data :
                        ...
                        preview_embed =discord .Embed (
                        title ="Current Welcome Configuration (Embed)",
                        color =0x000000 
                        )
                        preview_embed .add_field (name ="Title",value =data .get ("title","Not set"),inline =False )
                        preview_embed .add_field (name ="Description",value =data .get ("description","Not set"),inline =False )
                        preview_embed .add_field (name ="Color",value =data .get ("color","Not set"),inline =False )

                preview_embed .add_field (name ="Channel",value =channel .mention if channel else "Not set",inline =False )
            ...
            reconfigure_button =Button (label ="Reconfigure",style =discord .ButtonStyle .primary )
            reconfigure_button .callback =reconfigure_callback 
            setup_view .add_item (reconfigure_button )

            view_button =Button (label ="View Current",style =discord .ButtonStyle .secondary )
            view_button .callback =view_config_callback 
            setup_view .add_item (view_button )

            cancel_button =Button (label ="Cancel",style =discord .ButtonStyle .danger )
            cancel_button .callback =cancel_callback 
            setup_view .add_item (cancel_button )

            embed .set_footer (text ="Choose an option below to continue.")
            return await ctx .send (embed =embed ,view =setup_view )


        await self ._start_setup_flow (ctx )

    async def _start_setup_flow (self ,ctx ):
        """Starts the normal welcome setup flow for new configurations"""
    ...
    async def simple_setup (self ,ctx ):
        ...
        "joindate":ctx .author .joined_at .strftime ("%a, %b %d, %Y"),
    ...
    async def embed_setup (self ,ctx ):
        ...
        async def update_preview ():
            content =self .safe_format (embed_data ["message"],placeholders )or "Message Content."
            embed =discord .Embed (
            title =self .safe_format (embed_data ["title"],placeholders )or "",
            description =self .safe_format (embed_data ["description"],placeholders )or "```Customize your welcome embed, take help of variables.```",
            color =discord .Color (embed_data ["color"])if embed_data ["color"]else discord .Color (0x000000 )
            )

            if embed_data ["footer_text"]:
                embed .set_footer (text =self .safe_format (embed_data ["footer_text"],placeholders ),icon_url =self .safe_format (embed_data ["footer_icon"],placeholders )or None )
            if embed_data ["author_name"]:
                embed .set_author (name =self .safe_format (embed_data ["author_name"],placeholders ),icon_url =self .safe_format (embed_data ["author_icon"],placeholders )or None )
            if embed_data ["thumbnail"]:
                embed .set_thumbnail (url =self .safe_format (embed_data ["thumbnail"],placeholders ))
            if embed_data ["image"]:
                embed .set_image (url =self .safe_format (embed_data ["image"],placeholders ))
            ...
            try :
                if selected_option =="message":
                    ...
                    if selected_option in ["footer_icon","author_icon","thumbnail","image"]:
                        if url_or_text .startswith ("http")or url_or_text in ["{avatar}","{server_icon}"]:
                            embed_data [selected_option ]=url_or_text 
                        else :
                            await ctx .send ("Invalid URL. Please enter a valid image URL or a supported placeholder ({avatar} or {server_icon}).")
                    else :
                        embed_data [selected_option ]=url_or_text 
        ...
        async def submit_callback (interaction ):
            ...
            if not any (embed_data [key ]for key in ["title","description"]):
                await interaction .response .send_message ("Please provide at least a title or a description before submitting.",ephemeral =True )
                return 
    ...
    async def greet_reset (self ,ctx ):
        ...
        if not is_set_up :
            error =discord .Embed (description =f"No welcome message has been set for {ctx.guild.name}! Please set a welcome message first using `{ctx.prefix}welcome setup`",color =0x000000 )
            error .set_author (name ="Welcome is not configured!",icon_url ="https://cdn.discordapp.com/emojis/1337343347383074868.png")
            return await ctx .send (embed =error )

        embed =discord .Embed (
        title ="Are you sure?",
        description ="This will remove all welcome configurations & data related to welcome for this server!",
        color =0x000000 
        )
        ...
        async def yes_button_callback (interaction ):
            ...
            embed .title ="✅ Success"
            embed .description ="Welcome configuration has been successfully reset."
    ...
    async def greet_channel (self ,ctx ):
        ...
        def generate_view (page ):
            select_menu =Select (
            placeholder ="Select a channel for welcome",
            options =[
    ...
    async def greet_test (self ,ctx ):
        ...
        if not welcome_channel :
            error2 =discord .Embed (description =f"Welcome channel not set or invalid. Use `{ctx.prefix}welcome channel` to set one.",color =0x000000 )
            error2 .set_author (name ="Channel not set",icon_url ="https://cdn.discordapp.com/emojis/1337343347383074868.png")
        ...
        try :
            if welcome_type =="simple"and welcome_message :
                await welcome_channel .send (self .safe_format (welcome_message ,placeholders ))
                ...
                content =self .safe_format (embed_info .get ("message",""),placeholders )or None 
                embed =discord .Embed (
                title =self .safe_format (embed_info .get ("title",""),placeholders ),
                description =self .safe_format (embed_info .get ("description",""),placeholders ),
                color =embed_color 
                )
                ...
                if embed_info .get ("footer_text"):
                    embed .set_footer (
                    text =self .safe_format (embed_info ["footer_text"],placeholders ),
                    icon_url =self .safe_format (embed_info .get ("footer_icon",""),placeholders )
                    )
                if embed_info .get ("author_name"):
                    embed .set_author (
                    name =self .safe_format (embed_info ["author_name"],placeholders ),
                    icon_url =self .safe_format (embed_info .get ("author_icon",""),placeholders )
                    )
                if embed_info .get ("thumbnail"):
                    embed .set_thumbnail (url =self .safe_format (embed_info ["thumbnail"],placeholders ))
                if embed_info .get ("image"):
                    embed .set_image (url =self .safe_format (embed_info ["image"],placeholders ))

                await welcome_channel .send (content =content ,embed =embed )
        except discord .Forbidden :
            await ctx .send ("Failed to send test message: Missing permissions in the welcome channel.")
        except Exception as e :
            await ctx .send (f"An error occurred while testing: {e}")
    ...
    async def greet_config (self ,ctx ):
        ...
        if row :
            ...
            embed =discord .Embed (
            title =f"Welcome Configuration for {ctx.guild.name}",
            color =0x000000 
            )
            ...
            embed .add_field (name ="Welcome Channel",value =channel_display ,inline =False )
            embed .add_field (name ="Auto Delete Duration",value =auto_delete_duration ,inline =False )
            await ctx .send (embed =embed )
        else :
            error =discord .Embed (
            description =f"No welcome message has been set for {ctx.guild.name}! Please set a welcome message first using `{ctx.prefix}welcome setup`",
            color =0x000000 
            )
            error .set_author (name ="Greet is not configured!",icon_url ="https://cdn.discordapp.com/emojis/1337343347383074868.png")
            await ctx .send (embed =error )
    ...
    @greet .command (name ="edit",help ="Edits the current welcome settings for the server.")
    @commands .has_permissions (administrator =True )
    @commands .cooldown (1 ,6 ,commands .BucketType .user )
    @commands .max_concurrency (1 ,per =commands .BucketType .default ,wait =False )
    async def greet_edit (self ,ctx ):
        ...
        if row is None :
            error =discord .Embed (description =f"No welcome has been set for {ctx.guild.name}! Please set a welcome first using `{ctx.prefix}welcome setup`",color =0x000000 )
            error .set_author (name ="Greet is not configured!",icon_url ="https://cdn.discordapp.com/emojis/1337343347383074868.png")
            await ctx .send (embed =error )
            return 
        ...
        if welcome_type =="simple":
            ...
            async def edit_button_callback (interaction ):
                if interaction .user !=ctx .author :
                    await interaction .response .send_message ("You are not authorized to edit the welcome.",ephemeral =True )
                    return 

                await interaction .response .send_message ("Please provide the new welcome:",ephemeral =True )
                try :
                    ...
                    await interaction .message .edit (embed =embed ,view =view )
                    await ctx .send ("Welcome has been successfully updated.")
                except asyncio .TimeoutError :
            ...
            async def select_callback (interaction ):
                ...
                while not cancel_flag :
                    try :
                        if selected_option =="message":
                            await ctx .send ("Enter the welcome content:")
                            msg =await self .bot .wait_for ("message",check =lambda m :m .author ==ctx .author and m .channel ==ctx .channel ,timeout =600 )
