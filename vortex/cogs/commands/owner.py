...
from core import Cog ,VORTEXINF ,Context 
import sqlite3 
import os 
import requests 
...
class Owner (commands .Cog ):
    ...
    async def staff_add (self ,ctx ,user :discord .User ):
        if user .id in self .staff :
            aegis =discord .Embed (title ="⚠️ Access Denied",description =f"{user} is already in the staff list.",color =0x000000 )
            ...
            async with aiosqlite .connect (self .db_path )as db :
                ...
                await db .commit ()
            aegis2 =discord .Embed (title ="✅Success",description =f"Added {user} to the staff list.",color =0x000000 )
    ...
    async def staff_remove (self ,ctx ,user :discord .User ):
        if user .id not in self .staff :
            aegis =discord .Embed (title ="⚠️ Access Denied",description =f"{user} is not in the staff list.",color =0x000000 )
            await ctx .reply (embed =aegis ,mention_author =False )
        else :
            ...
            async with aiosqlite .connect (self .db_path )as db :
                ...
                await db .commit ()
                aegis2 =discord .Embed (title ="✅Success",description =f"Removed {user} from the staff list.",color =0x000000 )
            await ctx .reply (embed =aegis2 ,mention_author =False )
    ...
    async def staff_list (self ,ctx ):
        if not self .staff :
            ...
            staff_display ="\n".join (member_list )
            aegis =discord .Embed (title ="✅Quantum Staffs",description =f"\n{staff_display}",color =0x000000 )
    ...
    async def _slist (self ,ctx ):
        ...
        entries =entries ,
        description ="",
        title =f"Guild List of VORTEXINF-bot [{len(self.client.guilds)}]",
        color =0x000000 ,
        per_page =10 ),
        ctx =ctx )
    ...
    async def _restart (self ,ctx :Context ):
        await ctx .reply ("Rebooting VORTEXINF...")
        restart_program ()
    ...
    async def server_alert (self ,ctx ,*,message :str ):
        ...
        embed =discord .Embed (
        title ="🌸 VORTEXINF Update Time 🌸",
        description =message ,
        color =0x00d4ff ,
        timestamp =discord .utils .utcnow ()
        )
        embed .set_author (
        name ="Neva Development Team",
    ...
    async def own_list (self ,ctx ):
        ...
        paginator =Paginator (source =DescriptionEmbedPaginator (
        entries =entries ,
        title =f"VORTEXINF Owners [{len(nplist)}]",
        description ="",
        per_page =10 ,
        color =0x000000 ),
        ctx =ctx )
    ...
    async def _ownerban (self ,ctx :Context ,user_id :int ,*,reason :str ="No reason provided"):
        ...
        if member :
            try :
                ...
                embed =discord .Embed (
                description =f"⚠️ I do not have permission to ban **{member.name}** in this guild.",
                color =0x000000 
                )
    ...
    async def _ownerunban (self ,ctx :Context ,user_id :int ,*,reason :str ="No reason provided"):
        ...
        if user :
            try :
                ...
                embed =discord .Embed (
                description =f"⚠️ I do not have permission to unban **{user.name}** in this guild.",
                color =0x000000 
                )
                await ctx .reply (embed =embed ,mention_author =False )
            except discord .HTTPException :
