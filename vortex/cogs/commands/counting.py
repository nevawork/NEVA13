...
class Counting (commands .Cog ):
    ...
    async def on_message (self ,message ):
        ...
        try :
            ...
            if settings ['enforce_alternating']and counting_data ['last_user_id']==message .author .id :
                ...
                container .add_item (discord .ui .TextDisplay ("# ❌ Same User"))
    ...
    async def handle_wrong_count (self ,message ,guild_id :int ,attempted :int ,expected :int ):
        ...
        container .add_item (discord .ui .TextDisplay (f"# ❌ {await self.get_text(guild_id, 'wrong_count')}"))
        container .add_item (discord .ui .Separator ())
        ...
        if ctx .guild .text_channels :
            ...
            async def channel_callback (interaction :discord .Interaction ):
                try :
                    ...
                    await interaction .response .send_message (f"✅ **Channel Updated**\nCounting channel set to <#{channel_id}>",ephemeral =True )
                except Exception as e :
    ...
    async def counting_enable (self ,ctx ):
        ...
        try :
            ...
            container .add_item (discord .ui .TextDisplay (f"# ✅ {await self.get_text(ctx.guild.id, 'counting_enabled')}"))
            container .add_item (discord .ui .Separator ())
    ...
    async def counting_channel (self ,ctx ,channel :discord .TextChannel =None ):
        ...
        try :
            ...
            container .add_item (discord .ui .TextDisplay (f"# ✅ {await self.get_text(ctx.guild.id, 'channel_set')}"))
            container .add_item (discord .ui .Separator ())
    ...
    async def counting_info (self ,ctx ):
        ...
        try :
            ...
            container .add_item (discord .ui .Separator ())
            reset_icon ="✅"if settings ['reset_on_fail']else "❌"
            alt_icon ="✅"if settings ['enforce_alternating']else "❌"
