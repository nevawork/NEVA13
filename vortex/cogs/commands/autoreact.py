...
class AutoReaction (commands .Cog ):
    ...
    async def add (self ,ctx ,trigger :str ,*,emojis :str ):
        if len (trigger .split ())>1 :
            await self.send_message(ctx, title="❌ | Invalid Trigger", description="Triggers can only be one word.")
            return

        emoji_list =re .findall (r"<a?:\w+:\d+>|[\u263a-\U0001f645]",emojis )
        if len (emoji_list )>10 :
            await self.send_message(ctx, title="❌ | Too Many Emojis", description="You can only set up to **10** emojis per trigger.")
            return

        triggers =await self .get_triggers (ctx .guild .id )
        if len (triggers )>=10 :
            await self.send_message(ctx, title="⚠️ | Trigger Limit Reached", description="You can only set up to 10 triggers for auto-reactions in this guild.")
            return

        if await self .trigger_exists (ctx .guild .id ,trigger ):
            await self.send_message(ctx, title="⚠️ | Trigger Exists", description=f"The trigger '{trigger}' already exists. Remove it before adding it again.")
            return
        ...
        await self.send_message(ctx, title="✅| Trigger Added", description=f"Successfully added trigger '{trigger}' with emojis {', '.join(emoji_list)}.")
    ...
    async def remove (self ,ctx ,trigger :str ):
        if not await self .trigger_exists (ctx .guild .id ,trigger ):
            await self.send_message(ctx, title="❌ | Trigger Not Found", description=f"The trigger '{trigger}' does not exist.")
            return
        ...
        await self.send_message(ctx, title="✅| Trigger Removed", description=f"Successfully removed trigger '{trigger}'.")
    ...
    async def reset (self ,ctx ):
        triggers =await self .get_triggers (ctx .guild .id )
        if not triggers :
            await self.send_message(ctx, title="❌ | No Triggers Set", description="There are no auto-reaction triggers set to reset.")
            return
        ...
        await self.send_message(ctx, title="✅| All Triggers Reseted", description="Successfully removed all auto-reaction triggers.")
