...
class AutoRole (commands .Cog ):
    ...
    async def _autorole_humans_reset (self ,ctx ):
        ...
        if data and data [0 ]:
            async with aiosqlite .connect (DATABASE_PATH )as db :
                ...
                await db .commit ()
            await self.send_message(ctx, title="✅ Success", description="Cleared all human autoroles in this Guild.")
        else :
    ...
    async def _autorole_bots_reset (self ,ctx ):
        ...
        if data and data [0 ]:
            async with aiosqlite .connect (DATABASE_PATH )as db :
                ...
                await db .commit ()
            await self.send_message(ctx, title="✅ Success", description="Cleared all bot autoroles in this Guild.")
        else :
    ...
    async def _autorole_reset_all (self ,ctx ):
        ...
        if data and (data [0 ]or data [1 ]):
            async with aiosqlite .connect (DATABASE_PATH )as db :
                ...
                await db .commit ()
            await self.send_message(ctx, title="✅ Success", description="Cleared all autoroles in this Guild.")
        else :
    ...
    async def _autorole_humans_add (self ,ctx ,*,role :discord .Role ):
        ...
        if data :
            ...
            if role .id in humans :
                await self.send_message(ctx, title="⚠️ Access Denied", description=f"{role.mention} is already in human autoroles.")
            elif len (humans )>=10 :
                await self.send_message(ctx, title="⚠️ Access Denied", description="You can only add upto 10 human autoroles.")
            else :
                humans .append (role .id )
                async with aiosqlite .connect (DATABASE_PATH )as db :
                    await db .execute ("UPDATE autorole SET humans = ? WHERE guild_id = ?",(str (humans ),ctx .guild .id ))
                    await db .commit ()
                await self.send_message(ctx, title="✅ Success", description=f"{role.mention} has been added to human autoroles.")
        else :
            humans =[role .id ]
            async with aiosqlite .connect (DATABASE_PATH )as db :
        ...
        if data :
            ...
            if role .id not in humans :
                ...
                async with aiosqlite .connect (DATABASE_PATH )as db :
                    await db .execute ("UPDATE autorole SET humans = ? WHERE guild_id = ?",(str (humans ),ctx .guild .id ))
                    await db .commit ()
                await self.send_message(ctx, title="✅ Success", description=f"{role.mention} has been removed from human autoroles.")
        else :
    ...
    async def _autorole_bots_add (self ,ctx ,*,role :discord .Role ):
        ...
        if data :
            bots =eval (data [0 ])
            if role .id in bots :
                await self.send_message(ctx, title="⚠️ Access Denied", description=f"{role.mention} is already in bot autoroles.")
            elif len (bots )>=10 :
                await self.send_message(ctx, title="⚠️ Access Denied", description="You can only add upto 10 bot autoroles")
            else :
                bots .append (role .id )
                async with aiosqlite .connect (DATABASE_PATH )as db :
                    await db .execute ("UPDATE autorole SET bots = ? WHERE guild_id = ?",(str (bots ),ctx .guild .id ))
                    await db .commit ()
                await self.send_message(ctx, title="✅ Success", description=f"{role.mention} has been added to bot autoroles.")
        else :
            bots =[role .id ]
            async with aiosqlite .connect (DATABASE_PATH )as db :
        ...
        if data :
            ...
            if role .id not in bots :
                ...
                async with aiosqlite .connect (DATABASE_PATH )as db :
                    await db .execute ("UPDATE autorole SET bots = ? WHERE guild_id = ?",(str (bots ),ctx .guild .id ))
                    await db .commit ()
                await self.send_message(ctx, title="✅ Success", description=f"{role.mention} has been removed from bot autoroles.")
        else :
